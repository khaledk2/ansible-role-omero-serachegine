...
def test_nginx_gateway(host):
    out = host.check_output('curl  http://127.0.0.1:8080/searchengine/api/v1/resources/')
...
def test_searchengine_connection(host):
    out = host.check_output('curl  http://127.0.0.1:5577/searchengine/api/v1/resources/')
    assert 'OMERO search engine (API V1)' in out
...
#    out = host.check_output('curl  -k -u "elastic:elastic_password" https://127.0.0.1:9201/image_keyvalue_pair_metadata_1')
